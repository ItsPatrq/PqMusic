"""
W tym module znajdują się metody pomocnicze do rysowania wykresów
"""
import numpy as np
...
def getTimeTickLabels(secLength):
    fullSecs = int(math.floor(secLength))
    tickLabels = list(np.concatenate(
        ([0], np.arange(3, fullSecs - 3, 3), [round(secLength, 3)])))
    return tickLabels
...
def plot_spectrum_line_component_only(data, sampleRate, show=True, language="eng"):
    ...
    ax.plot(fr, X_m)
    ax.set_title(getStr(language, "spectrumLineComponent"),
                 fontsize=labelsFontSize)
...
def plot_correlation(data, sampleRate, language='eng'):
    ...
    plt.xticks(np.arange(0, len(data), 250), np.arange(
        0, len(data), 250), fontsize=ticksFontSize)
...
def plot_interpolated_correlation(interpolation, corelation, title="Correlation", language="eng"):
    ...
    plt.legend([getStr(language, "data"), getStr(
        language, "interpolated")], loc='best')
...
def plot_wave(normalized_data, sample_rate, wave_name, language="eng", x_axis_as_samples=False):
    ...
    if x_axis_as_samples:
        plt.xticks(np.arange(0, len(normalized_data), 250), np.arange(
            0, len(normalized_data), 250), fontsize=ticksFontSize)
        plt.plot(np.arange(0, len(normalized_data)), normalized_data)
        plt.xlabel(getStr(language, "timeSamples"), fontsize=labelsFontSize)

    else:
        plt.xticks(getTimeTickLabels(secLength), getTimeTickLabels(
            secLength), fontsize=ticksFontSize)
...
def plot_correlogram(data, spacing, sampleRate, show=True, showColorbar=True, language="eng"):
    ...
    H = np.array(data)
    image = ax.imshow(H.T, origin='lower', aspect='auto',
...
def plot_spectrogram(spectra, spacing, sampleRate, show=True, showColorbar=True, transpose=True, language="eng", title="spectrogram"):
    ...
    image = ax.imshow(spectra, interpolation='nearest',
                      origin='lower', aspect='auto')
    ax.set_yscale('log')
    ax.set_yticks(getFqTicks(sampleRate, frameWidth)[0])
    ax.set_yticklabels(getFqTicks(sampleRate, frameWidth)
                       [1], fontsize=ticksFontSize)
...
def plot_peaks(peaks, frameWidth, sampleRate, barwidth=3.0, show=True, language="eng"):
    ...
    ax.bar(peaks.nonzero()[0], peaks[peaks.nonzero()[0]],
           barwidth, color='black')
...
def plot_pitch_tracking(path, G):
    ...
    nx.draw_networkx_edge_labels(
        G, alpha=0.7, pos=pos, font_size=6, font_color='k')
