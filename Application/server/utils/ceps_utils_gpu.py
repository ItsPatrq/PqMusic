"""
W tym module znajdują się funkcje liczące cepstrum przy pomocy GPU
"""
...
def hanning_window(arr, NFFT):
    """
    Funkcja zwraca transformację, która stosuje okno Hanna do
    wierszy tablicy 2D. Aby uwzględnić wypełnienie zerowe (które nie
    ma być względnione w funkcji okna),  NFFT jest oddzielne.
    """
...
def rolling_frame(arr, NFFT, noverlap, pad_to):
    """
    Funkcja zwraca transformację, która przekształca tablicę 1D
    na tablicę 2D, której wiersze częściowo zachodzą na siebie.
    """
...
def log_pow(arr_t):
    """
    Zwraca transformację wykonującą log(abs(arr))
    """
...
def crop_frequencies_ceps(arr):
    """
    Zwraca transformację, która wycina dane częstotliwość
    Nyquista / 2  + zerowanie pierwszych 10 wartości cepstrum
    """
...
class CepsUtilsGpu(Computation):
    """
    Klasa pomocnicza do tworzenia planów liczenia cepstrum na GPU. Do inicjalizacji potrzebne
    są parametry danego sygnału oraz wielkość okna i margines zer.
    Aby plan został zainicjalizowany, potrzebne jest wywołanie instancji tej klasy
    z de wskaźnikiem na dane wejściowe na GPU oraz miejscem na wynik
    """
