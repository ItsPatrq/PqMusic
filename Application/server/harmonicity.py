...
sampleRate, data = pqutils.loadNormalizedSoundFIle(input_file)
all_weights = []
fft_len = frame_width*4
zeropad = np.zeros(frame_width*3)
best_frequencies = []

k0 = int(np.floor(pqutils.hz_to_fourier(f_0, frame_width*4, bitrate)))
k1 = int(np.ceil(pqutils.hz_to_fourier(f_1, frame_width*4, bitrate)))
...
pqutils.plot_pitches(best_frequencies, spacing, bitrate)

# display spectra:

plt.title("harmonicity-based loudness weights")
pqutils.plotSpectrogram(all_weights, fft_len, spacing, bitrate)
