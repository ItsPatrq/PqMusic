...
\hyphenation{FORTRAN Wi-ki-books pow-szech-ną któ-re-go pró-by}
...
Kompletna transkrypcja muzyczna jest złożonym zadaniem, które ze względu na specyfikę fal akustycznych możba podzielić na trzy pod-zadania:
\begin{itemize}
...
%\TODO: Artykuł znajduje się na 20-30 stronach journala, ale sam w sobie numerowany jest od 1 - 10. Jak odnosze się do 6 strony, powinno to być 6 czy zgodnie z journalem 26?
...
\end{itemize}
Metody ewaluacyjne przyjmują nazwę zbioru danych, z którego mają być brane dane oraz gemerują kombinacje argumentów wejściowych na podstawie zadeklarowanych słowników \textit{argsAc}, \textit{argsAclos} itd. Ze względu na to, że złożoność obliczeniowa rośnie wykładniczo wraz ze wzrostem możliwych wartości argumentów, algorytmy te wykorzystują multiprocesowość w celu polepszenia czasu wykonywania (co widać na listingu \ref{code:eval:multiproces}). 
...
with concurrent.futures.ProcessPoolExecutor() as executor:
  ...
  for arg in possibleArgsCombinations:
      procesess.append(executor.submit(validate_arguments, func, arg, evalObjects,
...
\begin{itemize}
  ...
  \item precyzja (ang. \textit{precision}) pokazująca jaka część klasyfikacji dotatniej jest prawdziwie dotatnia (\ref{eq:percision}),
...
Do ewaluacji algorytmów analizujących sygnał monofoniczny została wydzielona część danych bazy MAESTRO, a następnie zmodyfikowane zostały pliki MIDI tak, aby w danym momęcie w czasie obecna była tylko jedna nuta. Na podstawie tych plików zostały wygenerowane sygnały audio z wykorzystaniem sampli pianina Steinway. Dane przydzielone są losowo do grupy używanej w celu wyznaczenia najlepszych argumentów oraz do grupy, na której wykonywane są testy. 
...
W tablicy \ref{tab:bestArgMono} przedstawione są argumenty zaimplementowanych funkcji użyte podczas procesu ewaluacji dla sygnałów monofonicznych, którego wyniki w postaci wyżej opisanych miar statystycznych przedstawione są w tablicy \ref{tab:eval:mono}. Analogicznie, informacje o użytych alrgumentach i wynikach algorytmów dla sygnałów polifonicznych znajdują się w tablicach \ref{tab:bestArgPoli} i \ref{tab:eval:poli}. 
