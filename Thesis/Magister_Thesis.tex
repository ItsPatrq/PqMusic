...
W dalszej kolejności zostaną pokrótce opisane cztery wymienione wyżej cechy, które są głównym przedmiotem analizy w kierunku transkrypcji muzyki \cite[63]{Homerecording:DlaKazdego}.
\begin{figure}[h]
  \begin{center}
...
\subsection{Typy algorytmów transkrypcji muzyki}
\todo[inline]{Ta sekcja jest niedokończona}
Automatyczna transkrypcja muzyki jest nietrywialnym problemem, którego próby rozwiązania dotykają wielu dziedzin nauk informatycznych. Algorytmika, która została wypracowana do rozwiązania tego problemu jest różnorodna w wykorzystywanych metodologiach. Algorytmy te z biegiem czasu były modyfikowane w oparciu o nowe metody, krzyżowane z innymi oraz łączone w kompleksowe systemy. Przez ilość istniejących rozwiązań naturalnym jest kategoryzowanie ich ze względu na jakieś cechy charakterystyczne. Wielu badaczy w omawianej dziedzinie używa własnej nomenklatury do grupowania algorytmów i systemów transkrypcji muzyki ze względu na pewne zbiory cech. Niektóre z tych podziałów są powszechnie używane i za ich pomocą łatwo przedstawić działanie rdzenia danej metody. 
