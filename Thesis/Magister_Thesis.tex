...
W celu uproszczenia dostępu do zaimplementowanych algorytmów został zaimplementowany interfejs graficzny w postaci aplikacji webowej. W pliku \textit{server/server.py} znajduje się implementacja prostego serwera HTTP pozwalającego na wywoływanie opisanych algorytmów z wcześniej wyliczonymi najoptymalniejszymi parametrami. 

Implementacja GUI znajduje się w folderze \textit{static} i została napisana przy użyciu technologii \textit{React} i języka \textit{TypeScript}. Zawarty skrypt pozwala na zbudowania paczki statycznych plików, które wysyłane są przez serwer.
